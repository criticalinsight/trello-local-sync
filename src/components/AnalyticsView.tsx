        ...
        <div class="p-8 space-y-8 text-slate-300 max-w-7xl mx-auto">
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
                ...
                <div class="bg-slate-800/80 backdrop-blur border border-slate-700 p-6 rounded-2xl shadow-lg">
                    ...
                    <div class="text-4xl font-bold text-white">{Object.keys(stats().byModel).length}</div>
                </div>
            </div>
                            ...
                            .map(([model, count]) => (
                                <div>
                                    <div class="flex items-center justify-between mb-1">
                                        <span class="text-sm text-slate-300 font-medium">{model}</span>
                                    ...
                                    <div class="h-2 bg-slate-700 rounded-full overflow-hidden">
                                        <div
                                            ...
                                            style={{ width: `${(count / stats().totalRuns) * 100}%` }}
                                        />
                                    </div>
                                </div>
                            ))}
                ...
                <div class="bg-slate-800/80 backdrop-blur border border-slate-700 p-6 rounded-2xl shadow-lg">
                    ...
                    <div class="space-y-4">
                        <div class="flex items-center justify-between p-3 bg-slate-700/50 rounded-lg">
                            ...
                            <span class="font-mono text-white font-bold">{Object.values(promptStore.prompts).filter(p => p.status === 'queued').length}</span>
                        </div>
                        <div class="flex items-center justify-between p-3 bg-slate-700/50 rounded-lg">
                            <span>Generating Now</span>
                            <span class="font-mono text-emerald-400 font-bold">{Object.values(promptStore.prompts).filter(p => p.status === 'generating').length}</span>
                        </div>
                        <div class="flex items-center justify-between p-3 bg-slate-700/50 rounded-lg">
                            <span>Errors Last 24h</span>
                            <span class="font-mono text-red-400 font-bold">{Object.values(promptStore.versions).filter(v => v.error && v.createdAt > Date.now() - 86400000).length}</span>
                        </div>
                    </div>
                </div>
            </div>
