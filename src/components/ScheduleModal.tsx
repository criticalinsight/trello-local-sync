...
export const ScheduleModal: Component<ScheduleModalProps> = (props) => {
    ...
    const validateCron = (val: string) => {
        ...
        if (parts.length !== 5) return 'Cron must have exactly 5 segments (min hour day month weekday)';
        return '';
    };
        ...
        >
            <div class="bg-slate-900 border border-slate-700 rounded-2xl shadow-2xl w-full max-w-md overflow-hidden animate-in fade-in zoom-in duration-200">
                ...
                <div class="p-6 space-y-6">
                    <div class="space-y-2">
                        <label class="block text-sm font-medium text-slate-300">CRON Expression</label>
                        <input
                            type="text"
                            value={cron()}
                            onInput={(e) => { setCron(e.currentTarget.value); setError(''); }}
                        ...
                        <div class="mt-3 p-3 bg-slate-950/50 rounded-lg border border-slate-800">
                            <p class="text-[11px] text-slate-500 uppercase tracking-widest font-semibold mb-1">Examples</p>
                            <ul class="space-y-1 text-xs text-slate-400">
                                <li class="flex justify-between"><span>Every hour:</span> <code class="text-purple-300">0 * * * *</code></li>
                                <li class="flex justify-between"><span>Every midnight:</span> <code class="text-purple-300">0 0 * * *</code></li>
                                <li class="flex justify-between"><span>Every 15 mins:</span> <code class="text-purple-300">*/15 * * * *</code></li>
                            </ul>
                        </div>
                    </div>
